...
#ensemble name (specify as script argument in future)
...
path.out <- file.path(ensemble, "/reservoirs/")
...
swe.model.xts <- custom.read(file.path(path.in, "SNOW_Daily_Average_ByHRUGroup.csv"))
...
#### Process Precip Data ####
precip.model.xts <- custom.read(file.path(path.in, "PRECIP_Daily_Average_ByHRUGroup.csv"))
...
#precip.model.select %>% group_by(Year) %>% summarize(pp = sum(Precip, na.rm = T)) %>% View
# # # #### Calc inflows (from inflow_calculator script) - Ignore ET ####
# qnet.rfc <- read_csv("//mainfile-van/Projects/Active/3004430 Okanagan Flood Map/98 Received/RFC/20190919 full length inflows/2019-09-19allDates_inflows.csv") %>%
#       select(Date, qnet = Inflow_m3s) %>%
...
#          WY = ifelse(Month >= 10, Year + 1, Year)) %>%
#   group_by(Name, WY) %>%
#   mutate(Cumulative.in = cumsum(qnet)/1e6)

#inflows from model
#calculate Cumulative by water year
qnet <- read_csv(file.path(path.in, 'Hydrographs.csv')) %>%
...
# qnet_comp <- inner_join(qnet.rfc, qnet, by = c('Date', 'Name', 'WY')) %>%
#   group_by(WY, Name) %>%
#   summarize(ann.rfc = max(Cumulative.in),
#             ann.mod = max(Cumulative.in.mod)) %>%
#   mutate(diff = ann.rfc - ann.mod)
# #why the sine wave?
...
OBWB.forecast <- function(month, lake, year){
  ...
  if(month(forecast.date) == 3 & day(forecast.date) == 1){
    prevdate = ymd(paste(year, 2, 28, sep = '-'))
  } else {
    prevdate = forecast.date -days(1)
  }
  
  #get cumulative pp  
  precip.sum <- precip.model.select[Date == prevdate, Cumulative.pp]
  
  #get cumulative inflows
  inflow.sum <- qnet[Name == lake & Date == prevdate, Cumulative.in.mod]
...
okanagan.targets <- left_join(target.okanagan.df, forecast.vol.okanagan, by = c("Month", "Year"), all = TRUE) %>%
  mutate(Lake = "OkanaganLake", Target.Elevation = case_when(
    ...
    #Month == 4 & Forecast.Volume < 250 ~ 342.24 + ok.cgvd, #this is instead just the same as july
...
#     #Target.Elevation = Target.Elevation - 0.2)

# # #alternate future option, 20cm lower all winter.  may - sep stay the same
# target.okanagan.df <- tibble(Date = seq.Date(from = start_date, to = end_date, by = 1)) %>%
#   mutate(Year = year(Date),
#          Month = month(Date),
#          Day = day(Date),
#          Target.Elevation = case_when(
#     Month == 1 ~ 341.74 + ok.cgvd - 0.2,
#     Month == 5 ~ 342.48 + ok.cgvd,
#     Month == 6 ~ 342.44 + ok.cgvd,
#     Month == 7 ~ 342.24 + ok.cgvd,
#     Month == 8 ~ 342.04 + ok.cgvd,
# #    Month == 9 & Day < 23 & Day > 7 ~ 341.94  + ok.cgvd, #just use end of sept goals
#     Month == 9 ~ 341.89 + ok.cgvd,
#     TRUE ~ 341.84 + ok.cgvd - 0.2))
# 
# forecast.vol.okanagan <- forecast.volumes %>% filter(Lake == "OkanaganLake")
# 
# okanagan.targets <- left_join(target.okanagan.df, forecast.vol.okanagan, by = c("Month", "Year"), all = TRUE) %>%
#   mutate(Lake = "OkanaganLake", Target.Elevation = case_when(
#     Month ==2 & Forecast.Volume < 430 ~ 341.74 + ok.cgvd - 0.2, 
#     Month == 2 & Forecast.Volume >= 430 ~ 341.54 + ok.cgvd - 0.2,
#     Month == 3 & Forecast.Volume < 620 ~ 341.74 + ok.cgvd - 0.2, 
#     Month == 3 & Forecast.Volume >= 620 ~ 341.49 + ok.cgvd - 0.2,
#     Month == 4 & Forecast.Volume < 250 ~ 342.48 + ok.cgvd, 
#     Month == 4 & Forecast.Volume >= 250 & Forecast.Volume < 500 ~ 341.44 + ok.cgvd,
#     Month == 4 & Forecast.Volume >= 500 ~ 341.34 + ok.cgvd,
#     TRUE ~ Target.Elevation ))#,
...
#########################
#this doesn't work unless you start on the first of the month
###############################

start_seq = start_date
if(day(start_seq) != 1){
  day(start_seq) = 1
}

date.end.month <- c(start_seq, seq.Date(from = start_seq, to = end_date, by = 'months') - days(1), end_date)

date.end.month <- date.end.month[!(month(date.end.month) %in% c(2:3))]

#leap day doesnt matter because we dont use feb or march
# which((day(date.end.month) == 29) == T)
# 
# if(day(date.end.month)==29){
#   day(date.end.month) = 28
# }
...
kalamalka.targets <- left_join(target.kalamalka.df, forecast.vol.kalamalka, by = c("Month", "Year"), all = TRUE) %>%
  mutate(Lake = "KalamalkaLake",
         Target.Elevation = case_when(Month == 2 & Forecast.Volume > 15 ~ 391.2 + kal.cgvd,
                                      ...
                                      TRUE ~ Target.Elevation))#,
       #  Target.Elevation = Target.Elevation - 0.2) #future just make everything 20cm lower

# 
# #alternate future option, keeping value at minimum all winter, ignoring forecast
# target.kalamalka.df <- tibble(Date = seq.Date(from = start_date, to = end_date, by = 1)) %>% 
#   mutate(Year = year(Date),Month = month(Date), 
#          Day = day(Date), 
#          Target.Elevation = case_when(
#     Month == 1  ~ 391.0 + kal.cgvd, 
#     Month == 2  ~ 391.0 + kal.cgvd, #interpretation of keep it as high as possible
#     Month == 3  ~ 391.0 + kal.cgvd, #interpretation of keep it as high as possible
#     Month == 4  ~ 391.0 + kal.cgvd,             #april forecast decisiont
#     Month == 5  ~ 391.6 + kal.cgvd,
#     Month == 6  ~ 391.7 + kal.cgvd,
#     Month == 7  ~ 391.6 + kal.cgvd,
#     Month == 8  ~ 391.5 + kal.cgvd,
#     Month == 9  ~ 391.4 + kal.cgvd,
#     Month == 10 ~ 391.1 + kal.cgvd,
#     Month == 11 ~ 391.0 + kal.cgvd,
#     Month == 12 ~ 391.0 + kal.cgvd,
#     TRUE ~ NA_real_))
#        
# kalamalka.targets <- left_join(target.kalamalka.df, forecast.vol.kalamalka, by = c("Month", "Year"), all = TRUE) %>%
#    mutate(Lake = "KalamalkaLake")
...
#         Target.Max.Flow = 78)
         Target.Max.Flow = case_when(
           ...
           Month == 7  ~ 78,
           #Month == 8  ~ 28.3,
           Month == 8 ~ 78,  #breaking outflow rule to draw down farther, i.e. 1997
           #Month == 9  ~ 28.3,
           Month == 9 ~ 78,  #breaking outflow rule to draw down farther, i.e. 1997
           Month == 10 ~ 15.6,
           Month == 11 ~ 28.3,
           Month == 12 ~ 28.3,
           TRUE ~ NA_real_))
         #   Target.Max.Flow = 78) #all bets are off, always allow 78
...
okanagan.max.flow.targets <- left_join(okanagan.max.flow, forecast.vol.okanagan, by = c("Month", "Year"), all = TRUE) %>%
  mutate(Lake = "OkanaganLake", 
         Target.Max.Flow = case_when(Month == 4 & Forecast.Volume > 620 ~ 78,
                                     #Month == 9 & Day >14 ~ 15.6, #breaking outflow rule to draw down farther
...
okanagan.min.flow.targets <- left_join(okanagan.min.flow, forecast.vol.okanagan, by = c("Month", "Year"), all = TRUE) %>%
  mutate(Lake = "OkanaganLake", 
         Target.Min.Flow = case_when(Month == 4 & Forecast.Volume > 620 ~ 45,
                                     Month == 5 & Forecast.Volume > 620 ~ 45,
...
## Future runs have no leap days                      ##
########################################################

#filter out leap days
okanagan.targets <- okanagan.targets %>% filter(!(Month == 2 & day(Date) == 29))

cat(file = paste0(path.out, "/Okanagan_Reservoir_Targets.rvt"), append=F, sep="",
    ":ReservoirTargetStage 41","\n",
   format(okanagan.targets$Date[1], format = "%Y-%m-%d"), " 00:00:00.0", ' 1 ', nrow(okanagan.targets),"\n")
   write.table(okanagan.targets$Target.Elevation, paste0(path.out, "/Okanagan_Reservoir_Targets.rvt"),append=T,col.names=F,row.names=F,sep=" ",quote=F)
cat(file = paste0(path.out, "/Okanagan_Reservoir_Targets.rvt"), append=T, sep="",    ":EndReservoirTargetStage","\n")

#filter out leap days
okanagan.max.flow.targets <- okanagan.max.flow.targets %>% filter(!(Month == 2 & day(Date) == 29))

cat(file = paste0(path.out, "/Okanagan_Reservoir_MaxFlow.rvt"), append=F, sep="",
    ":ReservoirMaxFlow 41","\n",
    format(okanagan.max.flow.targets$Date[1], format = "%Y-%m-%d"), " 00:00:00.0", ' 1 ', nrow(okanagan.max.flow.targets),"\n")
write.table(okanagan.max.flow.targets$Target.Max.Flow, paste0(path.out, "/Okanagan_Reservoir_MaxFlow.rvt"), append=T,col.names=F,row.names=F,sep=" ",quote=F)
cat(file = paste0(path.out, "/Okanagan_Reservoir_MaxFlow.rvt"), append=T, sep="",    ":EndReservoirMaxFlow","\n")


#filter out leap days
okanagan.min.flow.targets <- okanagan.min.flow.targets %>% filter(!(Month == 2 & day(Date) == 29))

cat(file = paste0(path.out, "/Okanagan_Reservoir_MinFlow.rvt"), append=F, sep="",
    ":ReservoirMinFlow 41","\n",
    format(okanagan.min.flow.targets$Date[1], format = "%Y-%m-%d"), " 00:00:00.0", ' 1 ', nrow(okanagan.min.flow.targets),"\n")
write.table(okanagan.min.flow.targets$Target.Min.Flow,paste0(path.out, "/Okanagan_Reservoir_MinFlow.rvt"),append=T,col.names=F,row.names=F,sep=" ",quote=F)
cat(file = paste0(path.out, "/Okanagan_Reservoir_MinFlow.rvt"), append=T, sep="",    ":EndReservoirMinFlow","\n")


#filter out leap days
kalamalka.targets <- kalamalka.targets %>% filter(!(Month == 2 & day(Date) == 29))

cat(file = paste0(path.out, "/Kalamalka_Reservoir_Targets.rvt"), append=F, sep="",
    ":ReservoirTargetStage 42","\n",
    format(kalamalka.targets$Date[1], format = "%Y-%m-%d"), " 00:00:00.0", ' 1 ', nrow(kalamalka.targets),"\n")
write.table(kalamalka.targets$Target.Elevation, paste0(path.out, "/Kalamalka_Reservoir_Targets.rvt"),append=T,col.names=F,row.names=F,sep=" ",quote=F)
cat(file = paste0(path.out, "/Kalamalka_Reservoir_Targets.rvt"), append=T, sep="",    ":EndReservoirTargetStage","\n")
